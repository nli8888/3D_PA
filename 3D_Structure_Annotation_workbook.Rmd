...
Not_annotated$Diseased = "Pathogenic (missed by PA)"
All.distances = rbind(All.distances, Not_annotated)
All.distances$Diseased = factor(All.distances$Diseased, levels = c("Benign", "Pathogenic", "Pathogenic (missed by PA)"))
...
plot = ggplot(All.distances, aes(x=Diseased, y=Distance, color=Diseased)) + 
  ...
  scale_color_manual(values = c("Pathogenic"="#B85450", "Benign"="#82B366", "Pathogenic (missed by PA)" = "#0084ff")) +
  geom_jitter(width = 0.2, alpha = 0.5) + theme(axis.text.x=element_blank(), axis.ticks = element_blank()) + ylab("Distance (Angstrom)") +xlab("Variants") + theme(legend.title = element_blank())
ggsave("images/Closest_variant_annotation.png")
```
